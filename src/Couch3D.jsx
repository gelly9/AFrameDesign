...
// Facing south (+y): backrest on the north edge (cy - d/2).
export default function Couch3D() {
  const { cx, cy, w, d, h } = COUCH
  const innerW = w - 2 * ARM_W
  return (
    <group>
      {/* seat base */}
      <mesh position={[cx, SEAT_H / 2, cy + BACK_T / 2]} castShadow receiveShadow>
        <boxGeometry args={[innerW, SEAT_H, d - BACK_T]} />
        <meshStandardMaterial color={FABRIC} roughness={0.9} />
      </mesh>
      {/* backrest (north edge) */}
      <mesh position={[cx, h / 2, cy - d / 2 + BACK_T / 2]} castShadow>
      ...
      {/* seat cushions */}
      {[-1, 0, 1].map(i => (
        <mesh key={i} position={[cx + i * (innerW / 3), SEAT_H + 0.06, cy + BACK_T / 2]} castShadow>
